...
import org.apache.carbondata.store.core.exception.SchedulerException;
...
class Scheduler {
  ...
  private static Logger LOGGER = LogServiceFactory.getLogService(Scheduler.class.getName());
  ...
  private static final String CARBON_WORKER_WORKLOAD_LIMIT =
      "carbon.worker.workload.limit";

  private int getMaxWorkloadForWorker(int workerCores) {
    int defaultValue = workerCores * 10;
    try {
      return Integer.parseInt(
          CarbonProperties.getInstance().getProperty(
              CARBON_WORKER_WORKLOAD_LIMIT, String.valueOf(defaultValue)));
  ...
  Schedulable pickWorker(String splitAddress) {
    ...
    int maxWorkload = getMaxWorkloadForWorker(worker.getCores());
