...
class IndexedRecordReader extends CarbonRecordReader<CarbonRow> {
  ...
  public void initialize(InputSplit inputSplit, TaskAttemptContext context)
    ...
    if (fgDataMap != null) {
      queryModel = prune(table, queryModel, mbSplit, fgDataMap, context.getConfiguration());
    } else {
  ...
  private QueryModel prune(CarbonTable table, QueryModel queryModel,
      CarbonMultiBlockSplit mbSplit, DataMapExprWrapper datamap,
      ...
      if (uniqueSegments.get(segmentId) == null) {
        segments.add(Segment.toSegment(segmentId,
            new TableStatusReadCommittedScope(table.getAbsoluteTableIdentifier(),
                loadMetadataDetails, configuration)));
